...
export default function Hero({ channel, rowIndex, isFocused, registerRef, isFavorite, onSelect }) {
  ...
  return (
    <section className="relative mx-8 mb-4 overflow-hidden rounded-3xl border border-white/5 md:mx-12 lg:mx-16 animate-fade-in">
      ...
      <div className="relative grid gap-6 p-8 md:grid-cols-[1fr_auto] md:items-center md:p-12">
        <div className="max-w-xl">
          ...
          {hasEpg && now && (
            <p className="mt-4 text-sm text-slate-300">
              ...
              {next && <span className="text-muted"> · Urmează: {next.title}</span>}
              {now.stop && (
          ...
          <div className="mt-6 flex items-center gap-3">
            ...
            <span
              className={`rounded-full border px-4 py-3 text-sm ${
                fav ? 'border-focus/50 text-focus' : 'border-edge text-muted'
              }`}
            >
              {fav ? '★ Favorit' : '☆ apasă f'}
            </span>
        ...
        <div className="hidden h-40 w-56 place-items-center rounded-2xl bg-black/40 ring-1 ring-white/10 md:grid">
          {channel.logo ? (
            <img src={channel.logo} alt="" className="max-h-28 max-w-44 object-contain" />
          ) : (
